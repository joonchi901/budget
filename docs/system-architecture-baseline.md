...
| 자산·부채 | 예비금·투자금·전세금 등 일반 자산, 부채, 최초 잔액, 태그와 명시적 저축 집계 여부. 전체 대출 조건·월별 스냅샷은 후속 범위 |
...
- 자산 이동은 두 자산의 감소·증가를 함께 저장한다. 잔액 맞추기는 조회한 버전을 확인한 뒤 목표 잔액과 현재 잔액의 차이, 날짜·사유·작성자를 조정 이력으로 저장한다. 최초 잔액을 덮어쓰지 않는다.
