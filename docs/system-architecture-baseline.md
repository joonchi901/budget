...
| 검증 상태 | 현재 수입/지출·자산 변동·사용자 정의 태그 모델의 로컬 검증 완료. 범위와 실행 결과는 구현 현황 참조. 전체 기능·운영 배포·비용 실측 미완료 |

기능 범위의 기준은 [기능 정의서](/Users/joonchi/Budget/docs/functional-requirements.md)다. 본 문서는 해당 기능을 제공할 시스템 구성을 정의한다. v1.2는 수입/지출 원장, 명시적 자산 배분, 독립 자산 이동·잔액 조정, 저축 집계값과 사용자 정의 태그 모델을 반영한다. 원본의 모든 화면·데이터를 그대로 보존한다는 초기 가정보다 최신 기능 결정을 우선한다.

2026-09-17 계위·역할 갱신: [가계부 계위와 권한](./ledger-hierarchy.md). 외부 서비스 요금·한도 확인일은 기존 날짜를 유지한다.
...
| DEC-09 | 지출 귀속은 본인·배우자·공동으로 구분한다. 태그 유형·옵션은 사용자가 관리하며 이름·선택만으로 금액을 변경하지 않는다. |
...
| DEC-13 | 가계부 거래는 수입과 지출이다. 자산 이동·조정은 별도 이력이고 저축은 명시한 자산의 저장 당시 변동으로 집계한다. |
| DEC-14 | 자산 배분을 지정했다면 배분 합계는 거래 금액과 일치한다. 태그 조건에 따른 자동 배분은 후속 설정이며 고정 태그 규칙을 내장하지 않는다. |
...
- 현재 구현은 가구당 `HouseholdRoom` Durable Object 하나를 사용한다. 접속·선택 표시는 `ledgerId`로 구분하고 변경 순번은 같은 가구의 연결에 전달한다. 화면·가계부별 방 분리는 필요한 사용량 근거가 생겼을 때 재검토한다.
- 가계부는 단일 부모 트리이며 여러 최상위와 임의 깊이를 지원한다. 메인/목적 구분은 구버전 호환용이며 권한·집계 범위를 결정하지 않는다.
- 구조·역할 변경은 admin만 가능하다. `households.hierarchy_version`과 현재 역할을 변경과 같은 D1 원자적 배치에서 검사한다. 오래된 구조는 409로 거절하며 화면이 최신 구조를 보여준 뒤 사용자에게 재시도를 맡긴다.
...
| --- | --- |
| `1`~`12`, `월별시트 샘플페이지` | 수입·지출 입력과 사용자 정의 분류 태그; 날짜·내용·금액·결제수단·명시적 자산 배분 검증. 저축은 자산 통계로 분리 |
...
| 새 요구: 태그 및 귀속 | 본인·배우자·공동 지출 분석, 사용자 정의 태그 유형·옵션·범위·색상·순서·보관, 명시적 자산 배분과 별도 이동·조정 |
...
| D1 | 확정 데이터의 유일한 기준, 거래 소유·가계부 연결, 태그·거래 배분·자산 변동·저축 집계값, 수정 버전·변경 이력 | 가구·가계부·공유 자산·카드/통장·세션·변경 기록 |
...
- 거래는 정확히 하나의 원본 `ledger_id`에 속한다. 자산·부채·카드/통장·태그 유형과 자산 변동은 가구 공유 범위에 두며 메인 연결 여부와 독립적으로 유지한다.
...
| 거래 | 원본 가계부, 수입/지출, 거래일, 금액, 설명, 지출 귀속, 결제수단, 선택 옵션, 자산별 배분 |
| 계획 | 가계부별 월·주·목적 예산, 예산 항목·목표, 행사·경조사 일정 |
| 태그 | `tag_groups`의 단일/복수 선택·거래/자산 사용 위치·가계부 범위와 `tags`의 옵션·색상·순서·보관·버전 |
| 자산 변동 | `asset_operations`의 이동·조정 원본과 `asset_effects`의 실제 증감·날짜·사유·작성자·저축 집계값 |
| 자산·부채 | 예비금·투자금·전세금 등 일반 자산, 부채, 최초 잔액, 태그와 명시적 저축 집계 여부. 수동 대출 조건·관측 기준일·월별 잔액과 추이 |
...
- 자산 이동·조정은 가계부의 수입/지출과 별도 원본으로 기록한다. 저축은 저장된 자산 변동의 `savings_amount`를 합산하며 최초 잔액·조정·저축 대상 간 이동은 0으로 기록한다.
...
### 4.4 자산·저축·태그와 예산의 경계

- 태그 유형·옵션은 분류다. `selection_mode`로 단일/복수 선택을 검사하고 거래용 유형에는 전체 또는 지정 가계부 범위를 둔다. 자산용 유형은 공유 자산에 사용한다.
- 유형·옵션은 ID로 참조한다. 이름·색상·순서 변경과 보관은 기존 선택을 보존하며 자산 금액을 변경하지 않는다. 보관·범위 축소 후 새 선택을 제한하되 기존 내역은 조회할 수 있어야 한다.
- 거래 `allocations`가 비어 있으면 자산 효과가 없다. 배분을 지정하면 양의 금액·중복 없는 일반 자산·거래 금액과 같은 합계를 검사한다. 수입은 증가, 지출은 감소 효과를 저장한다.
- 자산 이동은 두 자산의 감소·증가를 함께 저장한다. 잔액 맞추기는 조회한 버전을 확인한 뒤 목표 잔액과 기록일 기준 잔액의 차이, 날짜·사유·작성자를 조정 이력으로 저장한다. 최초 잔액을 덮어쓰지 않는다.
- 자산의 `track_savings`는 명시적 집계 설정이다. 저장 시 효과에 `savings_tracking`과 `savings_amount`를 남겨 당시 해석을 보존한다. 현재 태그·이름·설정으로 과거 실적을 재계산하지 않는다.
- 최초 잔액·조정과 저축 집계 자산 간 이동은 저축 실적 0이다. 일반→저축은 유입, 저축→일반은 인출이며, 수입/지출 배분도 해당 자산의 저장 당시 설정에 따라 반영한다.
- 거래를 수정할 때 동일 자산의 기존 집계 여부는 유지하여 금액 효과를 교체한다. 새 자산에 배분하면 그 시점의 설정을 적용한다. 이동·조정을 취소할 때는 효과를 제거하고 원본의 취소 이력은 보존한다.
- 예산과 가계부 연결은 자산 금액·저축 실적을 바꾸지 않는다. 태그 조건에 따른 자동 배분은 후속 설정이며 초기의 고정 네 가지 규칙 모델을 사용하지 않는다.

#### 앱 내부 이전 데이터의 전환

`0002_tags_assets.sql`은 v1의 분류 문자열을 분류용 태그 옵션으로, 저축·이체 거래를 별도 자산 이동으로 전환한다. 새 조회는 수입/지출 원장과 `asset_effects`를 사용한다. 이전 테이블·감사 기록은 보존하며 자산별 잔액과 기존 저축 집계값을 이관한다. 이 작업은 원본 Excel 가져오기가 아니며, 실제 이관 결과와 재실행·중복 방지는 테스트 증거로 확인한다.

### 4.5 가계부 트리의 소유와 연결

- 모든 가계부는 독립적인 기간·예산·예산 항목·거래를 가지며 최상위 또는 다른 가계부 아래에서 사용할 수 있다. 전체 보기는 저장용 가계부가 아닌 가상 조회 범위다.
...
2. 서버는 세션·가구와 원본 가계부 권한·필수값·금액·태그 선택·명시적 자산 배분을 검증한다.
3. D1에서 기대 버전이 일치할 때만 갱신하고 버전을 증가시킨다. 읽은 뒤 무조건 덮어쓰는 방식은 사용하지 않는다.
4. 업무 데이터·자산 효과·저축 집계값·가구 변경 순번·변경 기록·성공 요청 기록을 하나의 원자적 작업으로 남긴다. 실제 자산 잔액이 바뀌었는데 원본 거래는 실패한 상태를 만들지 않는다.
5. D1 반영이 완료된 뒤 API가 저장 성공을 응답하고, 가구 협업방의 연결에 변경 순번을 알린다. 화면은 최신 데이터를 조회한다.
...
D1 저장과 WebSocket 전송은 하나의 트랜잭션이 아니다. 알림이 누락돼도 저장한 거래·자산 변동을 다시 생성하거나 취소하지 않는다.

- 가구의 변경 순번 `revision`을 저장·변경 이력과 함께 올린다. 접속 표시 자체는 원장 변경이 아니다.
- 현재 구현은 `/api/revision`으로 최신 순번을 확인하고, 차이가 있거나 재접속·탭 복귀·네트워크 복구가 발생하면 `/api/bootstrap`에서 전체 현재 상태를 다시 읽는다. 변경분 전용 API는 아직 없다.
- 최초 조회와 연결 사이에 생긴 변경도 연결 시 전달되는 순번과 재조회로 확인한다.
- 화면 활성 상태의 60초 순번 확인을 보완 경로로 사용하며 숨겨진 탭은 복귀 시 확인한다.
- 데이터 스냅샷과 순번은 일관된 조회에서 얻는다. 실제 적용이 끝난 순번만 클라이언트의 완료 상태로 기록하며 편집 중인 입력을 무조건 덮어쓰지 않는다.
- 중복·순서가 뒤바뀐 알림은 최신 상태 조회로 수렴시킨다. 큰 데이터에서 전체 조회의 전송량·읽기 사용량은 향후 측정하고 필요하면 변경분 조회를 추가한다.

별도 Queue·Cron 기반 알림 재전송은 두지 않는다. 정상 온라인 환경에서 누락 복구의 목표는 약 60초와 조회 소요 시간 이내이며 실제 원격 지연은 미측정이다.
...
- 가구당 협업방 하나에서 세션·가구 권한을 검사한다. 거래 화면의 접속·선택 위치는 `ledgerId`로 구분한다.
- 거래·자산·태그 설정 변경 후 같은 가구의 연결에 변경 순번을 보내고 현재 화면의 데이터를 다시 읽는다.
- 하위 원본과 메인·자산·카드 화면이 같은 거래를 조회해도 저장과 자산 효과는 한 번이다.
- 연결·해제는 메인 집계 범위만 바꾸며 자산 변동을 실행하지 않는다.
- 일부 연결의 알림이 누락돼도 순번 확인과 전체 상태 조회로 복구한다. 가계부 연결 상태나 방 ID 자체를 접근 권한으로 취급하지 않는다.
...
- 두 사람 모두 내역 편집과 조회가 가능하다. 본인은 admin, 배우자는 user로 시작한다. 생성·이동·순서 변경·보관·역할 관리·전체 백업 복원·엑셀 구조 가져오기는 admin만 가능하다. 마지막 admin의 강등을 거절한다. 지출 귀속과 사용자 권한은 별개다.
...
| Durable Objects | 요청 환산 100,000회/일, 실행 13,000 GB-s/일 | 현재 가구별 방 하나, Hibernation 적용 |
...
- 이 예시는 현재 가구별 방 하나의 계획용 수치다. 권한 조회, 전체 bootstrap 재조회, 태그·공유 자산 갱신을 포함한 실제 연산·읽기·요청 사용량은 별도로 실측한다.
...
9. 자산 항목의 초기 잔액·기준일과 거래 효과의 적용 범위를 구분한다. 원본에서 이미 반영된 거래를 자산 배분 과정에서 다시 차감하지 않는다. 앱 내부 v1→v2 마이그레이션과 Excel 이관은 별도로 검증한다.
...
코드 구조의 역할은 다음과 같다. 현재 파일과 완료 여부는 구현 현황에서 확인한다.
...
- 백업은 가구의 메인·목적 가계부, 거래 소유와 연결 관계, 공유 자산·카드/통장, 거래 배분·자산 이동/조정·저축 집계값·태그 설정·스키마 버전을 포함하고 로그인 세션·금융 인증정보는 제외한다. 복구 후 금융 연결은 다시 인증한다.
- 내보내는 동안 데이터가 바뀌면 일관된 스냅샷을 만들거나 재시도한다. 서로 다른 시점의 데이터를 한 백업으로 표시하지 않는다.
- 복원은 검증 환경에서 먼저 수행하고 거래 건수·합계·자산·카드/통장·가계부 연결·배분/자산 효과·저축 집계값·태그 설정을 비교한다. 복원된 거래에 이미 반영된 자산 효과를 다시 실행하지 않는다.
...
| AC-13 | 무료 용량 | 2명·2시간·1,000회 저장 시나리오에서 오류 없음; 로그인·일반 API 및 메인/목적/공유 화면의 가구 알림·재조회를 포함한 CPU·D1 행 수·DO 사용량 기록 |
...
| AC-20 | 배분·자산·저축 | 명시한 배분만 반영; 이동·조정은 별도 이력; 수정·삭제·취소와 효과가 정합하고 현재 태그·저축 설정이 과거 집계값을 다시 계산하지 않음 |
...
| AC-23 | 여러 화면 복구 | 하위 저장 알림이 일부 연결에 누락돼도 원본·메인·자산/카드 화면이 가구 revision 확인으로 같은 저장 상태에 수렴 |
...
| 가계부 연결 | 독립 예산·거래 단일 소유·재귀 합산 1회; 단일 부모·임의 깊이·여러 최상위; admin과 계위 버전 검사 | 2026-09-17 확정 |
| 태그·자산 배분 | 유형/옵션 관리와 명시적 배분 분리, 저장 당시 저축 집계, 수정·취소 정합성 | 후속 자동 배분 설정·소급 재적용을 설계할 때 범위 확정 |
...
| 무료 사용량 여유 | 공식 한도·한 방 예시 유지; 전체 재조회와 공유 자산/태그 갱신 비용 미측정 | 대표 기능 및 여러 화면 동시 사용 구현 후 |
...
2. **구성 검증:** 가상 데이터로 로그인, 거래·자산 변동·저축 집계값 원자 저장, 버전 충돌, 가구 변경 알림, 알림 누락·재연결 복구를 검증.
3. **데이터·화면 설계:** 메인·독립 목적 가계부, 통합 자산·카드/통장, 사용자 정의 태그·명시적 배분·자산 이동/조정, 모바일·PC 입력 흐름을 구체화.
...
| 버전 | 일자 | 내용 |
| --- | --- | --- |
| v1.1 | 2026-09-16 | 독립 목적 가계부와 초기 자산·태그 규칙 구조 반영 |
| v1.2 | 2026-09-16 | 수입/지출 원장, 사용자 정의 태그, 명시 배분, 별도 자산 변동·저축 집계값 및 현재 가구별 협업방 구성으로 개정 |
